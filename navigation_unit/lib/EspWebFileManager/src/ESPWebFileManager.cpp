...
#if FILE_MANAGER_USE_ASYNC_WEBSERVER == 1
  #include "SecData.h"
#endif
#include <SPIFFS.h>
#include <LittleFS.h>
...
void ESPWebFileManager::setServer(AsyncWebServer * server) {
  ...
  server -> on("/get-folder-contents", HTTP_GET, [ & ](AsyncWebServerRequest * request) {
    ...
    // Gestione richiesta di format per LittleFS
    server->on("/format-all", HTTP_GET, [](AsyncWebServerRequest *request) {
      if (!request->hasParam("confirm") || request->getParam("confirm")->value() != "true") {
          request->send(400, "application/json", "{\"error\":\"Missing or invalid confirm parameter\"}");
          return;
      }

      if (LittleFS.format()) {
          request->send(200, "application/json", "{\"status\":\"Format successful\"}");
      } else {
...
//   // Route to clear all files in the /log folder
// server->on("/clear-log", HTTP_POST, [&](AsyncWebServerRequest *request) {
//   String clearParam = request->hasParam("clear", true) ? request->getParam("clear", true)->value() : "";
  
//   if (clearParam != "1") {
//     request->send(400, "application/json", "{\"status\":\"error\",\"message\":\"Invalid clear parameter\"}");
//     return;
//   }

//   DEBUG_PRINT("Clearing /log folder");

//   String logPath = "/log";
  
//   if (!current_fs->exists(logPath)) {
//     request->send(404, "application/json", "{\"status\":\"error\",\"message\":\"Log folder not found\"}");
//     return;
//   }

//   File dir = current_fs->open(logPath);
//   if (!dir || !dir.isDirectory()) {
//     request->send(500, "application/json", "{\"status\":\"error\",\"message\":\"Failed to open log folder\"}");
//     return;
//   }
...
//       if (current_fs->remove(filePath)) {
//         deletedCount++;
//         DEBUG_PRINT2("Deleted: ", filePath);
//       } else {
//         failedCount++;
//         DEBUG_PRINT2("Failed to delete: ", filePath);
//       }
//     } else {
//       file.close();
//     }
...
//   String response = "{\"status\":\"success\",\"message\":\"Log cleared\",\"deleted\":" + 
//                     String(deletedCount) + ",\"failed\":" + String(failedCount) + "}";
//   request->send(200, "application/json", response);
// });


// Route to clear all files in a specified folder
server->on("/clear", HTTP_POST, [&](AsyncWebServerRequest *request) {
  String folderPath = request->hasParam("path", true) ? request->getParam("path", true)->value() : "";
  String clearParam = request->hasParam("clear", true) ? request->getParam("clear", true)->value() : "";
  ...
  DEBUG_PRINT2("Clearing folder: ", folderPath);

  if (!current_fs->exists(folderPath)) {
    request->send(404, "application/json", "{\"status\":\"error\",\"message\":\"Folder not found\"}");
    return;
  }

  File dir = current_fs->open(folderPath);
  if (!dir || !dir.isDirectory()) {
    request->send(500, "application/json", "{\"status\":\"error\",\"message\":\"Failed to open folder or path is not a directory\"}");
    return;
  }
  ...
  String response = "{\"status\":\"success\",\"message\":\"Folder cleared\",\"path\":\"" + 
                    folderPath + "\",\"deleted\":" + String(deletedCount) + 
